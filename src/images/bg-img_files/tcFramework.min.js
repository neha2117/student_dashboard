...
if(visitedPages.length>0){var data={pages:visitedPages,siteID:getSiteID(),url:document.URL,codeVersion:v3Lander.codeVersion};ROM.post(urls.resolvePageURL,data,null,null,function(context){if(context.data&&context.data.pages){this.pages=context.data.pages;this.onPageResolution();}}.bind(this));}};LE12.prototype._resolvePage=function(){var pages=this.pages;var pageURL=win.location.href;for(var i in pages){var re=pages[i].re;if(re===""){continue;}
...
this.dataLoaded=true;this.init();this.load();this.preloadInqFramework();if(this.isPersistentWindow()){openerCall("stopClosingChatFromPersistent");openerCall("registerPersistentWindowInOpener",[parent],false);this.setPersistentWindowActive(true);}
...
if(!c2c.isClicked()){this.fireC2CClicked(c2c);CHM.request(c2c.getRule(),c2c.getChatType(),c2c.getXmlChatSpec(),callThisNumber||null,c2c.c2p,idx,c2c.useNative);c2c.setClicked(true);X43.lastusedC2CId=c2c.pageElementID;X43.c2cBtnId=null;}
...
then=(then)?(then+(1000*this.getChatData().v3TO)):null;return then!=null&&then<now;};Chat.prototype.getVADataPass=function(){return this.vaDataPass;};Chat.prototype.setVADataPass=function(vaDataPass){this.vaDataPass=vaDataPass;};Chat.prototype.isAsyncReconnect=function(){return!!(this.cd&&this.cd.rec);}
function FlashPeer(id,data){var postponedAcifCommands=[];var nativeTimeout;function isV3C2CPersistent(){return!!v3C2cPersistent;}
...
dpObject["engagementID"]=CHM.getChatID();dpObject["agentID"]=getAgentID();ROM.sendDataToAgent(getAgentID(),dpObject);},getPreviousAsyncTranscript:function(callback){Glk4(callback);},isLastConversationActive:function(){return window.inqFrame.Application.application.checkActiveConversation();}}}
...
function SM19(surveyDataMap){var surveyURL=urls.baseURL+"/survey/genericSurvey.do";var NATIVE_SURVEY_EVENT="LoadPostChatSurvey";this.nativePostChatSurveyCall=null;this._observable();this.defaultWindowSpec="statusbar=0, scrollbars=1, resizable=1, toolbar=0, menubar=0, copyhistory=0";this.poppedWindows={};this.eventData=null;function getAltSurveyURL(data,evt){try{var brName=BRM.rb41(CHM.getLastChat().brID).getName();}catch(err){brName="NO BR Name Available";}
...
else{preSurveyDataPass=formDataAddition;}}};this.reset=function(){this.nativePostChatSurveyCall=null;};this.setEventDataForSurvey=function(eventData){this.eventData=eventData;}}
...
function setCustomerId(obj){if(typeof obj==="object"&&obj.data&&!inu1(obj.data.custID)){Inq.custID=obj.data.custID;Inq.save();BRM.fireOnResetSession();}}
